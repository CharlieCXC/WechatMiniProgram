...
describe('OrderService — create/accept/reject', () => {
  ...
  describe('createOrder', () => {
    ...
    it('rejects creating an order against a DISABLED sku', async () => {
      ...
      await expect(service.createOrder('u1', 'sku1')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('throws NotFound when sku missing', async () => {
      ...
      await expect(service.createOrder('u1', 'sku_nope')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('acceptOrder', () => {
    ...
    it('throws NotFound when order missing', async () => {
      ...
      await expect(service.acceptOrder('m1', 'x')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('rejects when caller is not the order master (IDOR)', async () => {
      ...
      await expect(service.acceptOrder('m1', 'o1')).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('rejectOrder', () => {
    ...
    it('rejects IDOR (master does not own order)', async () => {
      ...
      await expect(service.rejectOrder('m1', 'o1', 'x')).rejects.toThrow(
        NotFoundException,
      );
    });
