...
describe('ExtensionService', () => {
  ...
  describe('propose', () => {
    it('creates a PENDING extension request in IN_PROGRESS', async () => {
      ...
      const result = await service.propose('m1', 'o1', {
        additionalHours: 24,
        reason: '资料补充中',
      });
    ...
    it('rejects when there is already a PENDING extension', async () => {
      ...
      prisma.extensionRequest.findFirst.mockResolvedValue({
        id: 'ex_existing',
      });
      await expect(
        service.propose('m1', 'o1', { additionalHours: 24, reason: 'x' }),
  ...
  describe('respond', () => {
      ...
      prisma.$transaction.mockImplementation(async (fn) =>
        fn({
          extensionRequest: {
            update: jest
              .fn()
              .mockResolvedValue({ id: 'ex1', status: 'ACCEPTED' }),
          },
    ...
    it('REJECTED → no deadline change, decision card', async () => {
      ...
      prisma.extensionRequest.update.mockResolvedValue({
        id: 'ex1',
        status: 'REJECTED',
      });
    ...
    it('rejects when extension not PENDING', async () => {
      ...
      await expect(service.respond('u1', 'ex1', 'ACCEPTED')).rejects.toThrow(
        ConflictException,
      );
    });
