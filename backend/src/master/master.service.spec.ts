...
describe('MasterService', () => {
  ...
  beforeEach(async () => {
    prisma = { master: { findUnique: jest.fn(), create: jest.fn(), update: jest.fn() } };
    ...
    it('creates new master with PENDING status and placeholder profile', async () => {
      ...
      expect(prisma.master.create).toHaveBeenCalledWith({
        data: expect.objectContaining({
          phone: '13800138001', status: 'PENDING',
          displayName: '', avatar: '', intro: '', experience: '', philosophy: '',
          methods: [], topics: [],
        }),
      });
  ...
  describe('bindUnionid', () => {
    it('updates unionid for given master', async () => {
      ...
      expect(prisma.master.update).toHaveBeenCalledWith({ where: { id: 'm1' }, data: { unionid: 'wx_uni' } });
